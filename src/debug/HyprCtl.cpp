...
std::string CHyprCtl::getWindowData(PHLWINDOW w, eHyprCtlOutputFormat format) {
    ...
    "inhibitingIdle": {}
            ...
            w->m_bCurrentlySwallowing ? (uintptr_t)w->m_pSwallowed.lock().get() : 0, getFocusHistoryID(w), (g_pInputManager->isWindowInhibiting(w, false) ? "true" : "false"));
    } else {
        return std::format(
            "Window {:x} -> {}:\n\tmapped: {}\n\thidden: {}\n\tat: {},{}\n\tsize: {},{}\n\tworkspace: {} ({})\n\tfloating: {}\n\tpseudo: {}\n\tmonitor: {}\n\tclass: {}\n\ttitle: "
            "{}\n\tinitialClass: {}\n\tinitialTitle: {}\n\tpid: "
            ...
            (uint8_t)w->m_sFullscreenState.client, getGroupedData(w, format), getTagsData(w, format), w->m_bCurrentlySwallowing ? (uintptr_t)w->m_pSwallowed.lock().get() : 0, getFocusHistoryID(w),
